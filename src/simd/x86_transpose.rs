...
use crate::simd::SoaToAosPrev;
use crate::vec4u8::Vec4u8;

impl Default for SoaToAosPrev<__m128i> {
    fn default() -> Self {
        Self {
            prev0: unsafe { _mm_setzero_si128() },
            prev1: unsafe { _mm_setzero_si128() },
            prev2: unsafe { _mm_setzero_si128() },
            prev3: unsafe { _mm_setzero_si128() },
        }
    }
}
...
pub type SoaToAosPrevData = SoaToAosPrev<__m128i>;

#[inline]
pub fn load_m256i(src: &[u8; 32]) -> __m256i {
...
#[inline]
pub fn store_m256i(dst: &mut [u8; 32], val: __m256i) {
...
pub fn into_vec256(array: [u8; 32]) -> Vec256 {
    load_m256i(&array)
}

pub fn from_vec256(src: Vec256) -> [u8; 32] {
    let mut v: [u8; 32] = [0; 32];

    store_m256i(&mut v, src);
    v
}
...
pub fn prefix_sum_test(inp: Vec256) -> Vec256 {
    prefix_sum(inp, unsafe { _mm_setzero_si128() }).0
}

#[inline]
pub fn soa_to_aos_u8_32x4(
    input0: &[u8; 32],
    input1: &[u8; 32],
    input2: &[u8; 32],
    input3: &[u8; 32],
    out: &mut [Vec4u8; 32],
    mut prev: SoaToAosPrevData,
) -> SoaToAosPrevData {
    // SAFETY: simd/mod.rs exposes versions of this function for SSE2, SSSE3,
    // SSE4.1, AVX, and AVX2. Which version is used depends on compiler flags,
    // but there is no good way to plumb that through to avoid the unsafe.
    unsafe {
        ...
        (t0, prev.prev0) = prefix_sum(t0, prev.prev0);
        (t1, prev.prev1) = prefix_sum(t1, prev.prev1);
        (t2, prev.prev2) = prefix_sum(t2, prev.prev2);
        (t3, prev.prev3) = prefix_sum(t3, prev.prev3);
...
pub fn running_difference_test(inp: Vec256) -> Vec256 {
    let running_difference = running_difference_32(inp, 0);

    running_difference.0
}

#[inline]
pub fn aos_to_soa_u8_32x4(
